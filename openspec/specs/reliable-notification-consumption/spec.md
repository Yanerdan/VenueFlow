...
Define bounded, durable, and idempotent consumption of C11 Booking events by Notification
Service.
...
The consumer SHALL accept only UTF-8 JSON matching the C11 bounded envelope and the exact
`booking.reservation.confirmed.v1` or `booking.reservation.cancelled.v1` routing contract. It
...
- **WHEN** a C11 confirmation envelope and routing key agree and pass all bounds
...
queue, fixed-backoff retry queue, dead-letter exchange and queue, and exact bindings for the two
C11 Booking routing keys on the existing durable `venueflow.events.v1` topic exchange. Messages
and republished retry/dead-letter messages MUST be persistent.
...
Notification queues, and Notification MUST NOT modify the C11 exchange or event contract.
...
- **AND** C11 confirmation and cancellation routing keys are routable
