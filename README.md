...
本地启动会自动装载一套明确标注为合成数据的“2026 春季学期运营演示”：16 名跨院系人员、10 个校园资源、未来开放时段，以及 72 条覆盖约四个月的预约、审批与通知记录。再次执行 `scripts/local-dev/seed.ps1` 只会替换 `showcase-*`、`VF-CAMPUS-*` 和 `VF-SHOW-*` 保留命名空间中的演示记录，不会清除自行创建的数据。
