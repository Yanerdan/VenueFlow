...
资源卡片可维护归属部门，并从具备 `APPROVER` 或 `SYSTEM_ADMIN` 角色的人员下拉列表中选择审批人。新申请会快照该配置；`APPROVER` 只能查看并处理分配给自己的申请，`SYSTEM_ADMIN` 可处理全部申请。未配置审批人的历史资源仅由系统管理员兜底处理。
...
`smoke.ps1` 会创建一次性申请人、维护校园资料、提交完整用途申请，再以管理员写入审批意见并确认，通过运营报表、人员目录、角色升降级、合格审批人目录、搜索、通知、令牌刷新和退出完成全链路验证。验收账号最终会恢复为 `APPLICANT`。
