...
**VenueFlow 校园资源预约与审批治理平台｜个人全栈/微服务工程项目｜2026.XX–2026.XX**

面向高校部委、学院和职能部门的资源预约平台，统一管理场地发布、开放排期、师生申请、多级审批、通知和签到核销。个人完成需求建模、服务拆分、后端与双端 Web 开发、数据迁移、基础设施编排及自动化验收。

**技术栈：** Java 21、Spring Boot 4、Spring Cloud、Spring Security、MyBatis-Plus、MySQL、Redis、RabbitMQ、Elasticsearch、Nacos 3、Flyway、Docker Compose、OpenTelemetry
...
- 构建兼容固定地址与 Nacos 治理的双运行模式：自动初始化鉴权、命名空间和 8 个 Data ID，通过服务注册发现驱动 Gateway 路由与服务间调用，并以双 Resource 实例摘除、续读和恢复脚本验证故障切换。
- 实现 OIDC Authorization Code + PKCE、本地 Access/Refresh Token 轮换、组织架构全量/增量同步及角色和组织范围授权；在申请创建时固化审批链快照，确保后续配置变更不影响历史流程的审计语义。
- 使用 Flyway 管理 28 个数据库迁移，以 Docker Compose 和 PowerShell 完成环境启停、合成数据播种、静态/治理双模式验收及质量门禁；仓库维护 39 份可追踪规格，当前本地报告覆盖 278 项 Java 测试和 19 项前端测试。

> 简历空间有限时，建议保留前四条；第五条工程数字放在项目链接或面试材料中。
...
- 接入鉴权 Nacos 3 作为配置中心与注册中心，支持 Gateway 动态发现和 Resource 双实例负载均衡，并通过自动摘除、续读、恢复测试验证故障切换。
- 实现 OIDC + PKCE、JWT/Refresh Token 轮换、组织同步和角色/组织范围授权，并以审批链快照保持历史流程可审计。
- 使用 MySQL、Redis、Elasticsearch、Flyway、Docker Compose 和自动化脚本构建可重复启动、播种和验收的本地工程环境。

## 一行简介

独立设计并实现校园资源预约治理平台，以 Gateway + 6 个领域服务完成多级审批、容量一致性、可靠事件和校园身份治理，并通过 Nacos 双实例故障切换与自动化烟测验证微服务协作链路。

## 面试开场版

这是一个个人全栈工程项目，业务背景是学校场地由不同部门管理，申请规则和审批责任不一致。我重点解决四类问题：并发或重试时容量不能超卖；通知和搜索故障不能阻塞预约；审批配置变化不能改写历史流程；服务实例变化不能依赖手工改地址。对应实现了容量台账与补偿、事务 Outbox 与幂等消费、审批链快照，以及 Nacos 注册发现和双实例故障切换，同时补齐了校园身份、组织同步、双工作台和自动化验收。
...
- Added an optional authenticated Nacos 3 governance mode with repeatable namespace/Data ID bootstrap, service discovery, Gateway load balancing, and an automated two-instance Resource failover-and-recovery check.
...
- 不要说“Nacos 统一管理所有业务”；准确表述是“Nacos 提供配置中心与服务注册发现，Gateway 和领域服务仍负责路由与业务”。
- “故障切换通过”只表示本地双实例验收通过，不等同于生产高可用、跨机房容灾或性能结论。
