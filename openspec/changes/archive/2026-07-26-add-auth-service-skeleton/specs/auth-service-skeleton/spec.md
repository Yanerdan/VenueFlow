...
Auth Service MUST limit direct dependencies to Spring Boot Web MVC, Actuator, and test support.
It MUST NOT introduce Spring Security, JWT libraries, persistence, Flyway, database drivers,
Feign, Nacos, messaging, Redis, Elasticsearch, tracing exporters, Gateway, shared business
entities, or another service implementation module.
...
- **AND** no security, persistence, discovery, messaging, cache, search, tracing, or
  business-service dependency is resolved
...
startup MUST create no datasource, collaborator client, broker connection, cache client, or key
loader.

#### Scenario: Start without external infrastructure

- **WHEN** Docker, databases, brokers, caches, and every other service are unavailable
...
and architecture rules, HTTP probes, executable packaging, and real JAR startup without Docker or
another external service. Tests that start an application MUST use isolated ports and bounded
timeouts.

#### Scenario: Verify Auth Service without infrastructure

- **WHEN** CI or a developer runs default module or root Maven verification
- **THEN** all Auth Service tests complete without external connections
- **AND** no Testcontainers suite or infrastructure fixture starts
