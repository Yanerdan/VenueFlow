...
Exactly one concurrent caller MAY own a new `PROCESSING` request. A matching `SUCCEEDED` replay
MUST return the existing reservation, a matching `PROCESSING` request MUST NOT call
collaborators again, and a matching `FAILED` request MUST return its recorded safe failure.
Reuse of the same scoped key with another request hash MUST return a stable conflict before any
collaborator call.
...
Booking SHALL persist the reservation and mark idempotency `SUCCEEDED` in one local final
transaction after Resource allocation is proven. If that transaction fails, Booking MUST make
one release attempt using the deterministic release operation ID. Successful or already-applied
release SHALL be treated as compensated; failed or ambiguous release SHALL return
`BOOKING_COMPENSATION_REQUIRED` and leave a correlation-rich, secret-free operational record.

C10 MUST NOT claim durable automatic recovery for process termination or compensation failure.
...
- **AND** logs the request and operation correlations without exposing secrets

### Requirement: Reservation cancellation is state-idempotent and optimistic

Booking Service SHALL expose cancellation for a `CONFIRMED` reservation. It MUST ask Resource
to release the stored quantity with the stored deterministic release operation ID before
performing a conditional `CONFIRMED -> CANCELLED` update. The local transition MUST condition on
status and version. A cancelled replay SHALL return the same cancelled reservation, while a
release failure MUST leave the reservation confirmed.
